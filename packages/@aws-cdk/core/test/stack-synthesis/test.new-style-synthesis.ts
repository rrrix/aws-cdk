...
export = {
  ...
  'stack template is in asset manifest'(test: Test) {
    ...
    const manifest: cxschema.ManifestFile = JSON.parse(fs.readFileSync(manifestArtifact.file, { encoding: 'utf-8' }));
...
function readAssetManifest(asm: cxapi.CloudAssembly): cxschema.ManifestFile {
  const manifestArtifact = asm.artifacts.filter(isAssetManifest)[0];
